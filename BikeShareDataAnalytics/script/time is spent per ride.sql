...
select avg(SpentTime),TimeDay,Weekday from trip_fact
...
select avg(SpentTime) as avg_SpentTime,name from trip_fact
JOIN station_dimension ON(station_dimension.station_id=start_station_id)
group by name

/*Based on age of the rider at time of the ride*/
select avg(SpentTime) as avg_SpentTime,member from trip_fact
JOIN rider_dimension ON(rider_dimension.rider_id=member_id)
group by member

/*Based on whether the rider is a member or a casual rider*/
select avg(SpentTime) as avg_SpentTime,StartAge from trip_fact
JOIN rider_dimension ON(rider_dimension.rider_id=member_id)
group by StartAge
order by StartAge